...
protocol HealthTopicsApiProtocal{
    var services: ApiServiceable {get}
    typealias Response = (AnyObject?, HTTPURLResponse?, NSError?) -> Void
    func fetchHealthTopics<T: Decodable>(_ request: URLRequest?,for: T.Type, completion: @escaping (Swift.Result<T, Error>) -> Void)
    func fetchHealthTopicsDetail<T: Decodable>(_ request: URLRequest?,for: T.Type, completion: @escaping (Swift.Result<T, Error>) -> Void)
}
